...
export const resolvers = {
  Query: {
    async playersStats(parent, {player, sorting, recordsPerPage, page}: {player: string, sorting: SortInput[], recordsPerPage: RecordsPerPage, page: number}, context, info) {
      //Input validations
      if(page < 1){
      ...
      if(player != null && player.trim() !== ''){
        query.where("lower(stat.player) like :player", {player: `%${player.trim().toLowerCase()}%`})
      }
      ...
      if(sorting != null){
        sorting.forEach(s => {
          switch (s.field.trim().toLowerCase()) {
            ...
            default:
              throw new UserInputError("Invalid sorting field. Available options are: totalYards, longestRun and, td");
          }
        })
      }
      ...
      query.take(recordsPerPageInt).skip(recordsPerPageInt*(pageInt))
